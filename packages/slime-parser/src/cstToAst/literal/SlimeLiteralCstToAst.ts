...
export class SlimeLiteralCstToAstSingle {
    ...
    createNumericLiteralAst(cst: SubhutiCst): SlimeNumericLiteral {
        ...
        return SlimeAstCreateUtils.createNumericLiteral(Number(rawValue), rawValue)
    }
    ...
    createStringLiteralAst(cst: SubhutiCst): SlimeStringLiteral {
        ...
        // 保存原始值（raw）以保持引号格式
        const rawValue = cst.value as string
        const ast = SlimeAstCreateUtils.createStringLiteral(rawValue, cst.loc, rawValue)
        return ast
    }
    ...
    createLiteralAst(cst: SubhutiCst): SlimeLiteral {
        ...
        if (childName === SlimeTokenConsumer.prototype.NumericLiteral?.name || childName === 'NumericLiteral') {
            ...
            const rawValue = firstChild.value as string
            value = SlimeAstCreateUtils.createStringLiteral(rawValue, firstChild.loc, rawValue)
        }
        ...
        // 默认处理为字符串
            ...
            if (rawValue !== undefined) {
                ...
                const innerChild = firstChild.children?.[0]
                if (innerChild?.value) {
                    value = SlimeAstCreateUtils.createStringLiteral(innerChild.value, innerChild.loc, innerChild.value)
                } else {
    ...
    processTemplateMiddleList(cst: SubhutiCst, quasis: any[], expressions: SlimeExpression[]): void {
        ...
        for (let i = 0; i < cst.children.length; i++) {
            ...
            if (child.name === SlimeTokenConsumer.prototype.TemplateMiddle?.name ||
                ...
                quasis.push(SlimeAstCreateUtils.createTemplateElement(false, raw, cooked, child.loc))
    ...
    processTemplateSpans(cst: SubhutiCst, quasis: any[], expressions: SlimeExpression[]): void {
        ...
        if (first.name === SlimeTokenConsumer.prototype.TemplateTail?.name) {
            ...
            quasis.push(SlimeAstCreateUtils.createTemplateElement(true, raw, cooked, first.loc))
            return
        }
    ...
    createTemplateLiteralAst(cst: SubhutiCst): SlimeExpression {
        ...
        if (first.name === SlimeTokenConsumer.prototype.NoSubstitutionTemplate?.name ||
            ...
            const cooked = raw.slice(1, -1) // 去掉 ` �?`
            const quasis = [SlimeAstCreateUtils.createTemplateElement(true, raw, cooked, first.loc)]
            return SlimeAstCreateUtils.createTemplateLiteral(quasis, [], cst.loc)
        }
        ...
        return SlimeAstCreateUtils.createTemplateLiteral(quasis, expressions, cst.loc)
    }
